  ...
  head: () => ({
    meta: [
      { title: "MF SMC Trader — Forex SMC Analysis Platform" },
      {
        name: "description",
        content:
          "Professional forex trading terminal with live candlestick charts, multiple timeframes and Smart Money Concept tools.",
      },
    ],
  }),
  component: Terminal,
...
// Quick-access custom timeframes shown in the custom TF popover
const QUICK_CUSTOM_TFS = ["2m", "3m", "4m", "20m", "45m", "2h", "3h", "6h", "8h", "2d", "3d"];
...
function Terminal() {
  ...
  // Custom timeframe picker state
  const [customOpen,  setCustomOpen]  = useState(false);
  const [customInput, setCustomInput] = useState("");
  const [customError, setCustomError] = useState("");
  const customInputRef = useRef<HTMLInputElement>(null);

  const { candles, status, isLoading } = useMarketData(symbol, timeframeId);
  ...
  // Is the current timeframeId a custom (non-default) TF?
  const isCustomTf = !DEFAULT_TIMEFRAMES.find((t) => t.id === timeframeId);

  function applyCustomTf(id: string) {
    const parsed = parseCustomTimeframe(id.trim().toLowerCase());
    if (!parsed) {
      setCustomError("Invalid format. Try: 2h, 45m, 3d");
      return;
    }
    setTimeframeId(parsed.id);
    setCustomOpen(false);
    setCustomInput("");
    setCustomError("");
  }
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 border-b border-border bg-panel/60 px-4 py-2">
            ...
            <div className="relative ml-auto flex items-center gap-1">
              <div className="flex items-center gap-0.5 rounded-md border border-border bg-secondary/40 p-0.5">
                {DEFAULT_TIMEFRAMES.map((t) => (
                  <button
                    key={t.id}
                    onClick={() => setTimeframeId(t.id)}
                    className={cn(
                      "rounded px-2 py-1 text-xs font-semibold transition-colors",
                      t.id === timeframeId && !isCustomTf
                ...
                {/* Active custom TF chip */}
                {isCustomTf && (
                  <span className="flex items-center gap-0.5 rounded bg-primary px-2 py-1 text-xs font-semibold text-primary-foreground">
                    {tf.label}
                    <button
                      onClick={() => setTimeframeId("15m")}
                      aria-label="Remove custom timeframe"
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center rounded px-1.5 py-1 text-xs transition-colors",
                    customOpen
              ...
              {customOpen && (
                <div className="absolute right-0 top-full z-50 mt-1 w-64 rounded-lg border border-border bg-panel shadow-2xl">
                  ...
                  <div className="grid grid-cols-5 gap-1 p-3">
                    {QUICK_CUSTOM_TFS.map((q) => (
                      <button
                        key={q}
                        onClick={() => applyCustomTf(q)}
                  ...
                  <div className="border-t border-border px-3 py-2.5">
                    ...
                    <p className="mt-1 text-[10px] text-muted-foreground/60">
                      Format: <code>Nm</code>, <code>Nh</code>, <code>Nd</code> &nbsp;(e.g. 20m, 3h, 2d)
                    </p>
                  </div>
                </div>
              )}
...
import type { FeedStatus } from "@/hooks/use-market-data";
  ...
  return (
    <div className="flex items-center gap-1.5 rounded-full border border-border bg-secondary/50 px-2.5 py-1">
      {status === "connecting" ? (
        ...
        <span
          className={cn("h-2 w-2 rounded-full", meta.dot, meta.pulse && "live-dot")}
        />
      )}
